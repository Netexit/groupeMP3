...
class APEv2Parser extends BasicParser_1.BasicParser {
    ...
    static parseTagHeader(metadata, tokenizer, options) {
        return tokenizer.peekToken(APEv2Token_1.TagFooter).then(footer => {
            if (footer.ID === preamble) {
                return tokenizer.ignore(APEv2Token_1.TagFooter.len).then(() => {
                    return tokenizer.readToken(APEv2Token_1.TagField(footer)).then(tags => {
    ...
    parseHeader() {
        return this.tokenizer.readToken(APEv2Token_1.Header).then(header => {
            // ToDo before
